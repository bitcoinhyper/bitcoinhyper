...
#if defined(NDEBUG)
# error "Bitcoinhyper cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x8ca779aa4eb79c45f0c3a92231a278c0cbc6de2117e0a9bd8075de61d2cd6be8");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 2); // Bitcoinhyper: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Bitcoinhyper Signed Message:\n";
...
{
    // Bitcoinhyper: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 48000000); // Bitcoinhyper: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 10; // Bitcoinhyper: 10 seconds
static const int64 nTargetSpacing = 1 * 2; // Bitcoinhyper: 2 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Bitcoinhyper: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Bitcoinhyper: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x73e02c346fa12df3766e0696175b5a7e6a4f9e0670f37daa70101d78195ef9b7");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Washington Post Sep/28/2018 Kavanaugh nomination moves to initial vote - IN G-D WE TRUST";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04fcc5490770924ba66fbd53bda6f2be892b0fb976ed625b69d5cf66a68517fdc776ee0e998ade09d8ed29b26647571db2a4de3204c0daefc5c1561e081a155226") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1541863350;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084741261;

        if (fTestNet)
        {
            block.nTime    = 1541863301;
            block.nNonce   = 385430088;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x1c37447dd0c0f0f99d5a146e65d9c36f7432c492d750a75d68034a9655a65030"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc2, 0xb7, 0xd1 }; // Bitcoinhyper: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BitcoinhyperMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BitcoinhyperMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BitcoinhyperMiner(CWallet *pwallet)
{
    printf("BitcoinhyperMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bitcoinhyper-miner");
    ...
    try { loop {
        ...
        printf("Running BitcoinhyperMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BitcoinhyperMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BitcoinhyperMiner, pwallet));
}
